...
- **master 브랜치에 직접 커밋·푸시하지 않습니다.** 사소한 오타 수정이라도 예외 없음.
- 모든 변경 작업은 **새 브랜치를 만들어서** 진행하고, **PR(Pull Request)을 올려서** 머지합니다.
- 새 브랜치는 항상 `origin/master` 기준으로 생성합니다.
  ```
...
- PR 생성 전 `git log --oneline origin/master..HEAD`로 의도한 커밋만 포함되어 있는지 확인합니다.
...
예외는 없습니다. 사용자가 "이번엔 master에 바로 올려도 돼"처럼 **명시적으로** 요청한 경우에만 이 규칙을 건너뛸 수 있습니다. 명시적 지시가 없으면 항상 브랜치 + PR로 진행합니다.
